...
// Temporary verification functions until the utils are available
const isUserVerified = (user: any) => user?.verified || false;
const getVerificationBadgeText = (user: any) => 'PEAR Verified';
...
const MyCard: React.FC<MyCardProps> = ({ navigation, route }) => {
  ...
  const { currentLevel, nextLevel, xpToNext, progressPercent, addXP, getXPSummary, levelUpHistory } = enhancedXP;

  // Evolution animation state
  const [showEvolutionAnimation, setShowEvolutionAnimation] = useState(false);
  const [evolutionData, setEvolutionData] = useState<{ currentLevel: any; newLevel: any; } | null>(null);
  ...
  const getMetrics = () => {
    const userStats = getUserStatsByRole(role) as any;
    switch (role) {
      case 'trash-hero':
        return [
          { label: 'Jobs Completed', value: (userStats.jobsCompleted || 0).toString(), icon: 'briefcase', color: theme.primary },
          { label: 'Total Earned', value: userStats.totalEarned || '$0', icon: 'card', color: theme.primary },
          { label: 'Impact Radius', value: userStats.impactRadius || '0km', icon: 'location', color: theme.primary },
          { label: 'Success Rate', value: userStats.successRate || '0%', icon: 'shield-checkmark', color: theme.primary },
          { label: 'Hours Worked', value: userStats.hoursWorked || '0h', icon: 'time', color: theme.primary },
          { label: 'Verified Events', value: (userStats.verifiedEvents || 0).toString(), icon: 'shield-checkmark', color: '#10b981' },
        ];
      case 'impact-warrior':
        return [
          { label: 'Events Joined', value: (userStats.eventsJoined || 0).toString(), icon: 'people', color: theme.primary },
          { label: 'Impact Points', value: userStats.impactPoints || '0', icon: 'star', color: theme.primary },
          { label: 'Community Radius', value: userStats.communityRadius || '0km', icon: 'location', color: theme.primary },
          { label: 'Cleanups Led', value: (userStats.cleanupsLed || 0).toString(), icon: 'people-circle', color: theme.primary },
          { label: 'Volunteer Hours', value: userStats.volunteerHours || '0h', icon: 'time', color: theme.primary },
          { label: 'Verified Events', value: (userStats.verifiedEvents || 0).toString(), icon: 'shield-checkmark', color: '#10b981' },
        ];
      case 'business':
        return [
          { label: 'Funded Jobs', value: (userStats.fundedJobs || 0).toString(), icon: 'briefcase', color: theme.primary },
          { label: 'Total Investment', value: userStats.totalInvestment || '$0', icon: 'card', color: theme.primary },
          { label: 'Impact Radius', value: userStats.impactRadius || '0km', icon: 'location', color: theme.primary },
          { label: 'Jobs Created', value: (userStats.jobsCreated || 0).toString(), icon: 'people', color: theme.primary },
          { label: 'CO₂ Offset', value: userStats.co2Offset || '0kg', icon: 'leaf', color: theme.primary },
          { label: 'Verified Events', value: (userStats.verifiedEvents || 0).toString(), icon: 'shield-checkmark', color: '#10b981' },
        ];
      case 'admin':
        return [
          { label: 'Users Managed', value: (userStats.usersManaged || 0).toString(), icon: 'people', color: theme.primary },
          { label: 'System Uptime', value: userStats.systemUptime || '0%', icon: 'speedometer', color: theme.primary },
          { label: 'Platform Reach', value: userStats.platformReach || '0km', icon: 'location', color: theme.primary },
          { label: 'Issues Resolved', value: (userStats.issuesResolved || 0).toString(), icon: 'checkmark-circle', color: theme.primary },
          { label: 'Jobs Overseen', value: (userStats.jobsOverseen || 0).toString(), icon: 'eye', color: theme.primary },
          { label: 'Verified Events', value: (userStats.verifiedEvents || 0).toString(), icon: 'shield-checkmark', color: '#10b981' },
        ];
      default:
        return [
  ...
  const AttributeBar = ({ label, value, color }: { label: string; value: number; color: string; animationKey?: string }) => (
    ...
    >
      ...
      <View style={[styles.attributeBarBg, { backgroundColor: `${color}20` }]}>
        <View style={[styles.attributeBarFill, { backgroundColor: color, width: `${value}%` }]} />
      </View>
