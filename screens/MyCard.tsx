...
const MyCard: React.FC<MyCardProps> = ({ navigation, route }) => {
  ...
  const roleForData = role.toLowerCase().replace('_', '-') as
    | 'trash-hero'
    | 'impact-warrior'
    | 'eco-defender'
    | 'admin';
  ...
  useEffect(() => {
    if (levelUpHistory.length > 0) {
      ...
      const previousLevel =
        levelUpHistory.length > 1
          ? levelUpHistory[levelUpHistory.length - 2]
          : currentLevel;
  ...
  const getRoleSubtitle = (userRole: UserRole): string => {
    switch (userRole) {
      case 'TRASH_HERO':
        return 'Professional Cleaner';
      case 'IMPACT_WARRIOR':
        return 'Community Volunteer';
      case 'ECO_DEFENDER':
        return 'Environmental Investor';
      case 'ADMIN':
    ...
    switch (role) {
      case 'TRASH_HERO':
        return [
          ...
          {
            label: 'Hours Worked',
            value: stats.hoursWorked || '0h',
            icon: 'time',
            color: theme.primary,
          },
          {
            label: 'Verified Events',
            value: stats.verifiedEvents.toString(),
            icon: 'shield-checkmark',
            color: '#10b981',
          },
        ];
      case 'IMPACT_WARRIOR':
        return [
          ...
          },
          {
            ...
            value: stats.impactRadius || '0km',
            icon: 'location',
            color: theme.primary,
          },
          {
            label: 'Cleanups Led',
            value: stats.cleanupsLed?.toString() || '0',
            icon: 'people-circle',
            color: theme.primary,
          },
          {
            label: 'Volunteer Hours',
            value: stats.volunteerHours || '0h',
            icon: 'time',
            color: theme.primary,
          },
          {
      ...
      case 'ECO_DEFENDER':
        return [
          {
            label: 'Funded Jobs',
            value: stats.fundedJobs?.toString() || '0',
            icon: 'briefcase',
            color: theme.primary,
          },
          {
          ...
          {
            label: 'Jobs Created',
            value: stats.jobsCreated?.toString() || '0',
            icon: 'people',
            color: theme.primary,
          },
          {
            label: 'CO₂ Offset',
            value: stats.co2Offset || '0 tons',
            icon: 'leaf',
            color: theme.primary,
          },
          {
      ...
      case 'ADMIN':
        return [
          {
            label: 'Users Managed',
            value: stats.usersManaged?.toString() || '0',
            icon: 'people',
            color: theme.primary,
          },
          {
          ...
          },
          {
            ...
            value: stats.platformReach || 'Global',
            icon: 'location',
            color: theme.primary,
          },
          {
          ...
          {
            label: 'Jobs Overseen',
            value: stats.jobsOverseen?.toString() || '0',
            icon: 'eye',
            color: theme.primary,
          },
          {
  ...
  const attributes = {
    efficiency: Math.min(
      100,
      65 + Math.floor((xpState.totalXP || 0) / 100) * 2
    ),
  ...
  const AttributeBar = ({
    ...
    <TouchableOpacity
      ...
      onPress={() =>
        setSelectedAttribute(selectedAttribute === label ? null : label)
      }
      activeOpacity={0.7}
    >
      <View style={styles.attributeHeader}>
        <Text style={[styles.attributeLabel, { color: theme.textColor }]}>
          {label}
        </Text>
      ...
      {selectedAttribute === label && (
        <Text
          style={[styles.attributeDescription, { color: theme.secondaryText }]}
        >
  ...
  return (
    ...
    >
      ...
      <View style={styles.verificationSection}>
        {user && isUserVerified({...user, id: user.id.toString()}) ? (
          <View style={styles.verifiedBadge}>
            ...
            <Text style={[styles.verifiedText, { color: getRoleColor(role) }]}>
              {getVerificationBadgeText({...user, id: user.id.toString()})}
            </Text>
          </View>
        ) : (
          <TouchableOpacity
          ...
          >
            ...
            />
            <Text
              style={[styles.getVerifiedText, { color: getRoleColor(role) }]}
            >
              Get Verified
            </Text>
          </TouchableOpacity>
      ...
      <View style={[styles.myCard, { backgroundColor: theme.cardBackground }]}>
        ...
        <View style={styles.profileSection}>
          ...
          <View style={styles.profileInfo}>
            ...
            <Text
              style={[styles.badgeDescription, { color: theme.secondaryText }]}
            >
        ...
        <View style={styles.progressSection}>
          ...
          <View style={styles.progressFooter}>
            <Text
              style={[styles.progressPercentage, { color: theme.textColor }]}
            >
              {progressPercent}%
            </Text>
            <Text
              style={[styles.progressRemaining, { color: theme.secondaryText }]}
            >
        ...
        <View style={styles.attributesSection}>
          <View style={styles.sectionHeader}>
            ...
            <View
              style={[styles.liveBadge, { backgroundColor: roleConfig.color }]}
            >
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    setSelectedAttribute(
                      selectedAttribute === badge.id ? null : badge.id
                    )
                  }
                >
                  <Text style={styles.achievementIcon}>{badge.icon}</Text>
                  <Text
                    style={[styles.achievementName, { color: theme.textColor }]}
                  >
                    {badge.name}
                  </Text>
      ...
      <MenuModal
        ...
        userLevel={currentLevel.level || 1}
